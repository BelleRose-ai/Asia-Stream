...
export const HeroBanner: React.FC<HeroBannerProps> = ({ dramas, onSelectDrama }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 md:py-16 lg:py-20 flex flex-col justify-end min-h-[380px] sm:min-h-[460px] md:min-h-[520px]">
        <div className="max-w-2xl space-y-3 sm:space-y-4">
          {/* Badges */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-md text-[11px] sm:text-xs font-semibold bg-violet-600/90 text-white backdrop-blur-md shadow-lg">
              {drama.category}
            </span>
            <span className="px-2.5 py-0.5 rounded-md text-[11px] sm:text-xs font-semibold bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 flex items-center gap-1">
              <Star className="w-3 h-3 fill-cyan-400 text-cyan-400" />
              {drama.rating} Rating
            </span>
            <span className="px-2.5 py-0.5 rounded-md text-[11px] sm:text-xs font-medium bg-[#1a1b23]/80 text-gray-300 border border-[#2d2f39] flex items-center gap-1">
              <Calendar className="w-3 h-3 text-gray-400" />
              {drama.year}
            </span>
            <span className="px-2.5 py-0.5 rounded-md text-[11px] sm:text-xs font-medium bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
              {drama.episodesCount ? `${drama.episodesCount} Eps` : 'Feature'} ({drama.status || 'Ongoing'})
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 pt-0.5">
            {drama.genres.map((genre) => (
              <span key={genre} className="text-[11px] sm:text-xs px-2 py-0.5 rounded-full bg-[#1a1b23] text-gray-300 border border-[#2d2f39]">
                {genre}
              </span>
            ))}
          </div>
          ...
          {/* Synopsis */}
          <p className="text-xs sm:text-sm md:text-base text-gray-300 line-clamp-2 sm:line-clamp-3 leading-relaxed max-w-xl">
            {drama.synopsis}
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 pt-2 w-full sm:w-auto">
            ...
            <button
              onClick={() => onSelectDrama(drama)}
              style={{ minHeight: '44px', touchAction: 'manipulation' }}
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-[#1a1b23]/80 hover:bg-[#1a1b23] text-gray-300 hover:text-white font-medium text-xs sm:text-sm border border-[#2d2f39] hover:border-gray-500 flex items-center justify-center gap-2 transition-all cursor-pointer"
            >
        ...
        {validDramas.length > 1 && (
          <div 
            ...
            className="w-full pt-3 border-t border-[#2d2f39]/40 flex items-center justify-between sm:justify-end gap-4 z-10"
          >
              ...
              {validDramas.map((_, idx) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all cursor-pointer ${
                    idx === currentIndex ? 'bg-cyan-400 w-5' : 'bg-gray-600 hover:bg-gray-400 w-2'
                  }`}
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handlePrev}
                className="p-2 rounded-xl bg-[#1a1b23]/80 hover:bg-[#2d2f39] text-white border border-[#2d2f39] backdrop-blur-md transition-all shadow-lg cursor-pointer"
